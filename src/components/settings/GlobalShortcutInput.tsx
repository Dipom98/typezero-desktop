...
export const GlobalShortcutInput: React.FC<GlobalShortcutInputProps> = ({
  ...
  useEffect(() => {
    refreshSettings();
    // Force another refresh after a short delay to ensure backend is ready
  ...
  // Create a default binding object if it doesn't exist or bindings is empty
  // This allows us to set a new shortcut even if it's not in the settings yet
  const binding = bindings[shortcutId] || {
    name: t(`settings.general.shortcut.bindings.${shortcutId}.name`, "Toggle Dictation"),
    description: t(`settings.general.shortcut.bindings.${shortcutId}.description`, "Start/Stop Dictation"),
    current_binding: "",
  };
    ...
    >
      <div className="flex items-center space-x-1">
        {editingShortcutId === shortcutId ? (
          ...
          >
            {binding.current_binding ? formatKeyCombination(binding.current_binding, osType) : <span className="text-text-muted italic">Click to set</span>}
          </div>
        )}
        <ResetButton

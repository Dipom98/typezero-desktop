...
interface TypeZeroKeysShortcutInputProps {
...
export const TypeZeroKeysShortcutInput: React.FC<TypeZeroKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await (commands as any).stopTypezeroKeysRecording().catch(console.error);
  ...
  // Set up event listener for typezero-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<TypeZeroKeysEvent>(
        "typezero-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      (commands as any).stopTypezeroKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await (commands as any).startTypezeroKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {

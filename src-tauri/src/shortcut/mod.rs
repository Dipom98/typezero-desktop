...
//! - `typezero_keys`: Uses the typezero-keys library for more control
//!
//! The active implementation is determined by the `keyboard_implementation`
//! setting and can be changed at runtime.

mod handler;
pub mod typezero_keys;
mod tauri_impl;
...
// Note: Commands are accessed via shortcut::typezero_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => {
            if let Err(e) = typezero_keys::init_shortcuts(app) {
                error!("Failed to initialize typezero-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry TypeZeroKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => typezero_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => typezero_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => typezero_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => typezero_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (TypeZeroKeys needs state)
    if new_impl == KeyboardImplementation::TypeZeroKeys {
        if initialize_typezero_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => "typezero_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::TypeZeroKeys => typezero_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "typezero_keys" => KeyboardImplementation::TypeZeroKeys,
        other => {
            warn!(
...
/// Initialize TypeZeroKeys if not already initialized, with rollback on failure
fn initialize_typezero_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<typezero_keys::TypeZeroKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = typezero_keys::init_shortcuts(app) {
        error!("Failed to initialize TypeZeroKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize TypeZeroKeys: {}. Reverted to Tauri.",
            e
        ));
    }
...
#[tauri::command]
#[specta::specta]
pub fn change_telemetry_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_beta_channel_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.beta_channel_enabled = enabled;
    settings::write_settings(&app, settings);

    let _ = app.emit(
        "settings-changed",
        serde_json::json!({
            "setting": "beta_channel_enabled",
            "value": enabled
        }),
    );
...
pub fn change_app_theme_setting(app: AppHandle, theme: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    let parsed = match theme.as_str() {
        "dark" => settings::AppTheme::Dark,
        "plain" => settings::AppTheme::Plain,
        other => {
            warn!("Invalid app theme '{}', defaulting to dark", other);
    ...
    // Emit event to notify frontend of theme change
    let _ = app.emit(
        "settings-changed",
        serde_json::json!({
            "setting": "theme",
...
#[tauri::command]
#[specta::specta]
pub fn change_tts_voice_setting(app: AppHandle, voice: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_tts_speed_setting(app: AppHandle, speed: f64) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.tts_speed = speed as f32;
    settings::write_settings(&app, settings);
    Ok(())
}

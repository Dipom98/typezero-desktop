...
impl TranscriptionManager {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        self.transcribe_with_params(audio, None)
    }

    pub fn transcribe_with_params(
        &self,
        audio: Vec<f32>,
        custom_params: Option<TranscribeParams>,
    ) -> Result<String> {
        ...
        // Determine parameters (custom or settings-based)
        let (requested_language, requested_translate) = if let Some(params) = custom_params {
            (params.language, params.translate.unwrap_or(settings.translate_to_english))
        } else {
            (
        ...
        let result = {
            ...
            match engine {
                LoadedEngine::Whisper(whisper_engine) => {
                    ...
                    let whisper_language = if let Some(lang) = &requested_language {
                        let normalized = if lang == "zh-Hans" || lang == "zh-Hant" {
                            "zh".to_string()
                        } else {
                            lang.clone()
                        };
                ...
                LoadedEngine::SenseVoice(sense_voice_engine) => {
                    let lang_str = requested_language.as_deref().unwrap_or("auto");
        ...
        let translation_note = if requested_translate {
            " (translated)"
        } else {
            ""
        };

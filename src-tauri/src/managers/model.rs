...
impl ModelManager {
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        info!("download_model command received for ID: {}", model_id);
        let model_info = {
            let models = self.available_models.lock().unwrap();
            models.get(model_id).cloned()
        };
        ...
        let model_info =
            model_info.ok_or_else(|| {
                error!("Model not found in manager: {}", model_id);
                anyhow::anyhow!("Model not found: {}", model_id)
            })?;

        info!("Starting download_model for ID: {}", model_id);
        ...
        let mut response = match request.send().await {
            Ok(resp) => {
                info!("Network request successful for {}. Status: {}", model_id, resp.status());
                resp
            }
            Err(e) => {
                error!("Network request failed for {}: {}", model_id, e);
                return Err(e.into());
            }
        };
        ...
        info!("Creating/Opening partial file at: {:?}", partial_path);
        ...
        let mut file = if resume_from > 0 {
            match std::fs::OpenOptions::new()
                ...
                .open(&partial_path) {
                    Ok(f) => f,
                    Err(e) => {
                        error!("Failed to open partial file for appending: {}", e);
                        return Err(e.into());
                    }
                }
        } else {
            match std::fs::File::create(&partial_path) {
                Ok(f) => f,
                Err(e) => {
                    error!("Failed to create partial file: {}", e);
                    return Err(e.into());
                }
            }
        };
